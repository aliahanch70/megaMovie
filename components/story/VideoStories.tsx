    ...
    const fetchVideos = async () => {
      try {
        const { data, error } = await supabase
          .from('videos') // نام جدول
          .select('url'); // ستون URL رو می‌خوایم

        if (error) {
          console.error('خطا در گرفتن ویدیوها از Supabase:', error);
          return;
        }

        console.log('داده خام از جدول videos:', data);

        // استخراج URLها از داده‌ها
        const videoUrls = data.map((item) => item.url);
        console.log('لیست URLها:', videoUrls);
        setVideos(videoUrls);
      } catch (error) {
